    ...
    <div class="overflow-x-auto">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex flex-col md:flex-row md:justify-between md:items-end gap-4 px-6 py-5">
                ...
                @if (auth()->user()?->area_id == 1 || auth()->user()->role == 'admin' || auth()->user()->id == 6  || auth()->user()->id == 3 || auth()->user()->id == 5)
                                    ...
                                    @forelse ($tickets as $ticket)
                                        <tr>
                                            ...
                                            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                                    @if ($ticket->osticket)
                                                        <a href="https://osticket.cechriza.com/scp/tickets.php?a=search&search-type=typeahead&query={{ $ticket->osticket }}&number={{ $ticket->osticket }}"
                                            ...
                                            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    @if (auth()->user()->role === 'admin')
                                                        <button wire:key="anular-ticket-{{ $ticket->id }}"
            ...
            Livewire.on('mostrarSwalAnulacion', (data) => {
                Swal.fire({
                    ...
                    preConfirm: (comentario) => {
                        if (!comentario) {
                            Swal.showValidationMessage(
                                'Debes ingresar un comentario antes de continuar.');
                            return false;
                        }
                        return comentario;
                    }
                    ...
                    if (result.isConfirmed && result.value) {
                        Livewire.dispatch('anularTicketConComentario', {
                            id: data.id,
