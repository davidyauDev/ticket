<div class="p-6">
    <header class="mb-12 flex flex-col md:flex-row md:items-end md:justify-between">
        ...
        <div class="mt-4 flex items-center gap-3 md:mt-0">
            <button
                class="inline-flex items-center justify-center whitespace-nowrap font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border bg-background hover:text-accent-foreground rounded-md px-3 h-8 gap-1 border-gray-200 text-xs text-gray-600 hover:bg-gray-50">
    ...
    <div class="p-4 space-y-4">
        <div class="border-b border-gray-100 px-6 pt-4">
            <div role="tablist" aria-orientation="horizontal"
                class="h-10 items-center justify-center rounded-md text-muted-foreground grid w-full grid-cols-2 gap-4 bg-transparent p-0"
                tabindex="0" data-orientation="horizontal" style="outline: none;">
                <button type="button" role="tab" aria-selected="{{ $tab === 'mis' ? 'true' : 'false' }}"
                    aria-controls="radix-«r0»-content-mis-tickets"
                    data-state="{{ $tab === 'mis' ? 'active' : 'inactive' }}" id="radix-«r0»-trigger-mis-tickets"
                    class="inline-flex items-center justify-center whitespace-nowrap rounded-sm ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:shadow-sm border-b-2 border-transparent px-0 py-2 text-sm font-normal text-gray-500 hover:text-gray-900 data-[state=active]:border-gray-900 data-[state=active]:text-gray-900"
                    tabindex="0" data-orientation="horizontal" data-radix-collection-item=""
                    wire:click="$set('tab', 'mis')">Mis Tickets</button>
                <button type="button" role="tab" aria-selected="{{ $tab === 'todos' ? 'true' : 'false' }}"
                    aria-controls="radix-«r0»-content-todos-tickets"
                    data-state="{{ $tab === 'todos' ? 'active' : 'inactive' }}" id="radix-«r0»-trigger-todos-tickets"
                    class="inline-flex items-center justify-center whitespace-nowrap rounded-sm ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:shadow-sm border-b-2 border-transparent px-0 py-2 text-sm font-normal text-gray-500 hover:text-gray-900 data-[state=active]:border-gray-900 data-[state=active]:text-gray-900"
                    tabindex="-1" data-orientation="horizontal" data-radix-collection-item=""
                    wire:click="$set('tab', 'todos')">Todos los Tickets</button>
            </div>
        </div>
